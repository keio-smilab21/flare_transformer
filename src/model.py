...
#         self.magnetogram_module = InformerEncoderLayer(
#             AttentionLayer(ProbAttention(False, factor=5, attention_dropout=mm_params["dropout"], output_attention=False),
#                            d_model=mm_params["d_model"]*2*window, n_heads=mm_params["h"], mix=False),
#             mm_params["d_model"]*2*window,
#             mm_params["d_ff"],
#             dropout=mm_params["dropout"],
#             activation="relu"
#         )
...
#         self.sunspot_feature_module = InformerEncoderLayer(
#             AttentionLayer(ProbAttention(False, factor=5, attention_dropout=sfm_params["dropout"], output_attention=False),
#                            d_model=sfm_params["d_model"]*2*window, n_heads=sfm_params["h"], mix=False),
#             sfm_params["d_model"]*2*window,
#             sfm_params["d_ff"],
#             dropout=sfm_params["dropout"],
#             activation="relu"
#         )
...
#         # SFM 1 di
#         phys_input = torch.flatten(phys_input, 1, 2).unsqueeze(1)  # [bs, 1, k*(SFM+MM)_d_model]
#         print(phys_input.shape)
#         feat_output = self.sunspot_feature_module(phys_input)
#         feat_output = torch.flatten(feat_output, 1, 2)  # [bs, k*SFM_d_model]
#         feat_output = self.generator_phys(feat_output)  # [bs, SFM_d_model]
#         # print(feat_output.shape)
#         # sys.exit()

#         img_input = torch.flatten(img_input, 1, 2).unsqueeze(1)  # [bs, 1, k*(SFM+MM)_d_model]
#         # print(img_input.shape)
#         img_output = self.magnetogram_module(img_input)
...
#         self.conv1 = nn.Conv1d(in_channels=window*2,
#                                out_channels=1, kernel_size=1)
#         self.conv2 = nn.Conv1d(in_channels=window*2,
#                                out_channels=1, kernel_size=1)
...
#         # SFM 1 di
#         phys_input = self.conv1(phys_input)  # [bs, 1, d_model]
#         print(phys_input.shape)
#         feat_output = self.sunspot_feature_module(phys_input)
#         feat_output = torch.flatten(feat_output, 1, 2)  # [bs, k*SFM_d_model]
#         # feat_output = self.generator_phys(feat_output)  # [bs, SFM_d_model]
...
#         # self.magnetogram_module = InformerEncoderLayer(
#         #     AttentionLayer(ProbAttention(False, factor=5, attention_dropout=mm_params["dropout"], output_attention=False),
#         #                    d_model=(sfm_params["d_model"] + mm_params["d_model"])*window, n_heads=mm_params["h"], mix=False),
#         #     (sfm_params["d_model"] + mm_params["d_model"])*window,
#         #     mm_params["d_ff"],
#         #     dropout=mm_params["dropout"],
#         #     activation="relu"
#         # )
...
#         # self.sunspot_feature_module = InformerEncoderLayer(
#         #     AttentionLayer(ProbAttention(False, factor=5, attention_dropout=sfm_params["dropout"], output_attention=False),
#         #                    d_model=(sfm_params["d_model"] + mm_params["d_model"])*window, n_heads=sfm_params["h"], mix=False),
#         #     (sfm_params["d_model"] + mm_params["d_model"])*window,
#         #     sfm_params["d_ff"],
#         #     dropout=sfm_params["dropout"],
#         #     activation="relu"
